        ...
        */
        ...
        var core = {
            ...
            render: function (element) {
                ...
                if ((element.pageNum + 1) < Math.ceil(element.rowsNum / settings.itemsPerPage))
                    $dataPanel.css({ height: $leftPanel.height() });
                else
                    $dataPanel.css({ height: $leftPanel.height() - tools.getCellSize() }); //substract for fake row
            ...
            leftPanel: function (element) {
                ...
                $.each(element.data, function (i, entry) {
                    if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        entries.push('<div class="row name row' + i + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + i + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '" data-id="' + entry.id + '">');
                ...
                switch (settings.scale) {
                    ...
                    case "hours":
                        ...
                        for (var i = 0, len = range.length; i < len; i++) {
                            ...
                            if (tools.isPostponedHoliday(rday)) {
                            ...
                            if (rgetDay !== getDay) {
                                var day_class2 = (today - day === 0) ? "today" : tools.isPostponedHoliday(day.getTime()) ? "postponedHoliday" : dowClass[getDay];
                        ...
                        if (tools.isPostponedHoliday(day)) {
                            day_class = "postponedHoliday";
                        }
                    ...
                    default:
                        ...
                        var weekNum = myWeekCalc(range[0]);
                        var weekNumNext;
                        var daysInWeekNum = 1;
                        ...
                        for (var i = 0, len = range.length; i < len ; i++) {
                            ...
                            weekNumNext = myWeekCalc(rday);
                            if (weekNum == weekNumNext) {
                                daysInWeekNum++;
                        ...
                        $.each(entry.values, function (j, day) {
                            ...
                            switch (settings.scale) {
                                ...
                                case "hours":
                                    ...
                                    _bar.css({ 'top': top + 24, 'left': Math.floor(cFrom) });
                                ...
                                default:
                                    ...
                                    var from = $(element).find("#dh-" + dFrom);
                                    var cFrom = from.attr("offset");
                                    var data_id = $(element).find("#rowheader" + i).attr("data-id");

                                    var dl = Math.round(((dTo / 1000) - (dFrom / 1000)) / 86400) + 1;
                                    _bar = core.createProgressBar(
                                                dl,
                                                day.customClass ? day.customClass : "",
                                                day.desc ? day.desc : "",
                                                day.label ? day.label : "",
                                                day.dataObj ? day.dataObj : null
                                        ).attr({ dayFrom: dFrom, dayTo: dTo, data_id: data_id, desc: day.desc, sick_desc: day.sickType });
        ...
        var tools = {
            ...
            isPostponedHoliday: (function () { // IIFE
                // short-circuits the function if no holidays option was passed
                if (!settings.postponedHolidays) {
                    return function () { return false; };
                }

                var postponedHolidays = false;

                // returns the function that will be used to check for holidayness of a given date
                return function (date) {
                    if (!postponedHolidays) {
                        postponedHolidays = tools._datesToDays(settings.postponedHolidays);
                    }

                    return !!postponedHolidays[
                     // assumes numeric dates are already normalized to start-of-day
                      $.isNumeric(date) ?
                        date :
                      (new Date(date.getFullYear(), date.getMonth(), date.getDate())).getTime()
        ...
        this.each(function () {
            ...
            switch (settings.scale) {
                ...
                default: this.headerRows = 5; this.scaleStep = 13; break;
            }
