    ...
    public static class DisplayBTsDatesActionLink
    {
        public static MvcHtmlString CustomDisplayBTsDatesActionLink(this HtmlHelper helper, BusinessTrip businessTrip, string selectedDepartment = "")
        {
            if (businessTrip != null)
            {
                ...
                {
                    return new MvcHtmlString(String.Format("<a id=\"EditReportedBTACC\" href=\"{0}{1}?selectedDepartment={2}\" data-date-format=\"{7}\"> <blue><b>{5} - {6}</b></blue> <orange>{8}</orange> &nbsp; &nbsp; {3} - {4} </a>", "/BusinessTrip/EditReportedBT/", businessTrip.BusinessTripID, selectedDepartment, businessTrip.StartDate.ToShortDateString(), businessTrip.EndDate.ToShortDateString(), businessTrip.OrderStartDate.Value.ToShortDateString(), businessTrip.OrderEndDate.Value.ToShortDateString(), dateFormat, businessTrip.DaysInBtForOrder));
                }
                else
                {
                    return new MvcHtmlString(String.Format("<a id=\"EditReportedBTACC\" href=\"{0}{1}?selectedDepartment={2}\" data-date-format=\"{5}\"> {3} - {4} </a>", "/BusinessTrip/EditReportedBT/", businessTrip.BusinessTripID, selectedDepartment, businessTrip.StartDate.ToShortDateString(), businessTrip.EndDate.ToShortDateString(), dateFormat));
                }
            }
