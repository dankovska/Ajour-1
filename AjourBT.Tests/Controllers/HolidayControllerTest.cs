...
namespace AjourBT.Tests.Controllers
{
    ...
    public class HolidayControllerTest
    {
        ...
        public void EditPost_CanEdit_ValidHolidayFalse()
        {
            // Arrange - create the controller 
            HolidayController target = new HolidayController(mock.Object);
            Holiday holiday = new Holiday { HolidayID = 1, CountryID = 1, HolidayComment = "Test Comment", HolidayDate = new DateTime(2014, 01, 01), Title = "New Year" };


            // Act - call the action method 
            var result = (RedirectToRouteResult)target.Edit(holiday, "2014.05.05", 1, false);
            ...
            mock.Verify(m => m.SaveHoliday(holiday, new DateTime(2014, 05, 05), 1, false), Times.Once);
        }
        ...
        public void EditPost_CannotEdit_InvalidHolidayCheckboxFalse()
        {
            ...
            var result = target.Edit(holiday, "2014.01.01", 1,false);
            string data = (string)(new Microsoft.VisualStudio.TestTools.UnitTesting.PrivateObject(((JsonResult)result).Data, "error")).Target;

            // Assert - check the result 
            mock.Verify(m => m.SaveHoliday(holiday, new DateTime(2014, 04, 04), 1, false), Times.Never);
        ...
        public void CreatePost_CanCreate_ValidHolidayCheckboxTrue()
        {
            // Arrange - create the controller                 
            HolidayController target = new HolidayController(mock.Object);
            Holiday Holiday = new Holiday { HolidayID = 1, Title = " Test", HolidayDate = new DateTime(2014,01,01)};

            // Act - call the action method 
            RedirectToRouteResult result = (RedirectToRouteResult)target.Create(Holiday, "01.01.2014", 1, true);

            // Assert - check the result 
            mock.Verify(m => m.SaveHoliday(Holiday, new DateTime(2014, 01, 01), 1, true), Times.Once);
            Assert.IsFalse(result.Permanent);
            Assert.AreEqual("Home", result.RouteValues["controller"]);
            Assert.AreEqual("ABMView", result.RouteValues["action"]);
        }
        ...
        public void CreatePost_CannotCreate_InvalidHolidayChecboxFalse()
        {
            ...
            // Act - call the action method 
            target.ModelState.AddModelError("error", "error");
            ViewResult result = target.Create(holiday, "", 1, false) as ViewResult;

            // Assert - check the result 
            mock.Verify(m => m.SaveHoliday(It.IsAny<Holiday>(), new DateTime(2014, 01, 01), 1, false), Times.Never);
            Assert.IsInstanceOf(typeof(Holiday), result.ViewData.Model);
            Assert.IsInstanceOf(typeof(ViewResult), result);
        }
